...
def dashboard():
   ...
   # Get filter and sort parameters from the request
    ...
    sort_param = request.args.get('sort', 'added_date')

    # Create the base query joining the User and Task tables
    base_query = db.session.query(Task).join(User).filter(User.id == current_user.id)

    # Apply filtering based on the filter parameter to the base query
    if filter_param == 'completed':
        base_query = base_query.filter(Task.completed == True)
    elif filter_param == 'active':
        base_query = base_query.filter(Task.completed == False)
    elif filter_param in ['low', 'medium', 'high']:
        base_query = base_query.filter(Task.category == filter_param)

    # Apply sorting based on the sort parameter to the tasks
    if sort_param == 'due_date':
        tasks = base_query.order_by(Task.due_date).all()
    else:
        tasks = base_query.order_by(Task.due_date).all()

    return render_template("dashboard.html", tasks=tasks, form=form, current_user=current_user)

# my route for loggin out
@app.route("/logout")
@login_required
def logout():
    logout_user()
...
# my route for toggling the task completed checkbox 
